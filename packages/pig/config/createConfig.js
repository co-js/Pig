...
function configFactory(env = 'dev', { host = 'localhost', port = 3000, modify, plugins, modifyBabelOptions }, webpackEntity) {
  ...
  const babelOptions = modifyBabelOptions ? modifyBabelOptions(babelOptions) : babelOptions;
  ...
  const { raw, stringified } = getEnv({ host, port });
  const devServerPort = parseInt(raw.PORT, 10) + 1;
  const clientPublicPath = raw.CLIENT_PUBLIC_PATH || (IS_DEV ? `http://${raw.HOST}:${devServerPort}` : '/');
